                    ...
                    if has_different_stresses:
                        ...
                        for stress_form in sx:
                            ...
                            if stress_pos != stressed_word.new_stress_pos:
                                ...
                                new_stressed_word = WordStressVariant(stressed_word.poetry_word, stress_pos,
                                                                      stressed_word.get_score())
    ...
    def get_stress_variants(self, aligner):
        ...
        if uform == 'начала' and self.upos == 'NOUN':
            ...
            for accent in aligner.accentuator.ambiguous_accents2[uform]:
                ...
                if stress_pos == -1:
                    raise ValueError(
                        'Could not find stressed position in word "{}" in ambiguous_accents2'.format(accent))

                variants.append(WordStressVariant(self, stress_pos, 1.0))
        elif len(self.alternative_stress_positions) > 1:
            # не удалось обработать омограф будем перебирать все варианты ударения в нем.
            ...
            # разрешаем становится безударными односложным глаголам.
            variants.append(WordStressVariant(self, self.stress_pos, 1.0))
            variants.append(WordStressVariant(self, -1, 0.7))
        elif uform == 'нет':
            # частицу "нет" с ударением штрафуем
            ...
            if uform in ('о', 'у', 'из', 'от', 'под', 'подо', 'за', 'при', 'до', 'про', 'для', 'ко', 'со', 'во', 'на',
                         'по') and self.upos == 'ADP':
                # эти предлоги никогда не делаем ударными
                variants.append(WordStressVariant(self, -1, 1.0))
                # Но если это последнее слово в строке, то допускается
                if self.is_rhyming_word:
                    variants.append(WordStressVariant(self, self.stress_pos, 1.0))
                else:
                ...
                if self.is_rhyming_word:
                    ...
                    variants.append(WordStressVariant(self, self.stress_pos, 0.70))
                else:
                ...
                if count_vowels(uform) < 3:
                    ...
                    if uform in ['лишь', 'вроде', 'если', 'чтобы', 'когда', 'просто', 'мимо', 'даже', 'всё', 'хотя',
                                 'едва', 'нет', 'пока']:
                        variants.append(WordStressVariant(self, self.stress_pos, COEFF['@68_2']))
                    else:
                        variants.append(WordStressVariant(self, self.stress_pos, COEFF['@68']))
            ...
            if nvowels == 1:
                ...
                if uform in ['эти', 'эту', 'это', 'мои', 'твои', 'моих', 'твоих', 'моим', 'твоим', 'моей', 'твоей',
                             'мою', 'твою', 'его', 'ее', 'её', 'себе', 'меня', 'тебя', 'свою', 'свои', 'своим', 'они',
                             'она',
                             'уже', 'этом', 'тебе']:
...
class LineStressVariant(object):
    ...
    def score_sequence(self, aligner):
        ...
        # штрафуем за безударное местоимение после безударного предлога с гласной:
        ...
        for word1, word2 in zip(self.stressed_words, self.stressed_words[1:]):
            # проверяем цепочки согласных в смежных словах
            ...
            # Штрафуем за ударный предлог перед существительным:
            if word1.poetry_word.upos == 'ADP' and word1.new_stress_pos > 0 and word2.poetry_word.upos in (
            'NOUN', 'PROPN') and word2.new_stress_pos > 0:
                self.total_score *= 0.5
                self.penalties.append('@317')

        if sum(self.stress_signature) == 1:
            # Всего один ударный слог в строке с > 2 слогов - плохо
            # одно слово или группа предлог+сущ - нормально
            if len(self.poetry_line.pwords) > 2 or (
                    len(self.poetry_line.pwords) == 2 and self.poetry_line.pwords[-2].upos != 'ADP'):
...
class PoetryLine(object):
    ...
    def build(text, udpipe_parser, accentuator):
        ...
        # Ограждаем символы пунктуации пробелами, чтобы они гарантировано не слиплись со словом
        for c in '\'.‚,?!:;…-–—«»″”“„‘’`ʹ"˝[]‹›·<>*/=()+®©‛¨×№\u05f4':
                    ...
                    if count_vowels(word) > 0 and stress_pos == -1:
                        ...
                        if stress_pos == -1:
                            if word in accentuator.ambiguous_accents:
                                # Слово является омографом берем все варианты ударения.
                                for stressed_form, tagsets in accentuator.ambiguous_accents[word].items():
                                    i = locate_Astress_pos(stressed_form)
                                    alt_stress_pos.append(i)
                                stress_pos = alt_stress_pos[0]
    ...
    def locate_rhyming_word(self):
        # Отмечаем последнее слово в строке,так как оно часто ударное
        located = False
    ...
    def get_stress_variants(self, aligner):
        ...
        # добавляем варианты, возникающие из-за особых ударений в словосочетаниях типа "пО полю"
        lwords = [w.form.lower() for w in self.pwords]
        if any((w in aligner.collocation2_first) for w in lwords) and any(
                (w in aligner.collocation2_second) for w in lwords):
        ...
        # добавляем варианты для триграмм типа "бок О бок"
        if any((w in aligner.collocation3_first) for w in lwords) and any(
                (w in aligner.collocation3_second) for w in lwords) and any(
                (w in aligner.collocation3_third) for w in lwords):
            add_variants = []
            for colloc in aligner.collocations:
                if len(colloc) == 3:
...
class PoetryStressAligner(object):
    ...
    def check_rhyming(self, rhyming_tail1, rhyming_tail2):
        ...
        r = rhymed2(self.accentuator,
                    ...
                    [poetry_word1.poetry_word.upos] + poetry_word1.poetry_word.tags, rhyming_tail1.prefix,
                    unstressed_tail1,
                    poetry_word2.poetry_word.form, poetry_word2.new_stress_pos,
                    [poetry_word2.poetry_word.upos] + poetry_word2.poetry_word.tags, rhyming_tail2.prefix,
                    unstressed_tail2)
        if r:
            return True
    ...
    def _align_line_groups(self, line_groups):
        # Для каждого эталонного метра проверяем, насколько группы строк вписываются в этот метр
        best_score = -1.0
        best_meter = None
    ...
    def detect_repeating(self, alignment, strict=False):
        ...
        # Иногда модель выдает повторы существительных типа "любовь и любовь" в одной строке.
        for pline in alignment.poetry_lines:
            ...
            if strict:
                # Повтор слова длиннее 4 букв  считаем плохим
            ...
            for w1, w2, w3 in zip(pline.poetry_line.pwords, pline.poetry_line.pwords[1:], pline.poetry_line.pwords[2:]):
                if w2.form.replace('\u0301', '') in ('и', ',', 'или', 'иль', 'аль', 'да'):
                    if w1.form.replace('\u0301', '').lower() == w3.form.replace('\u0301',
                                                                                '').lower() and w1.form.replace(
                            '\u0301',
                            '') not in 'вновь еще ещё снова опять дальше ближе сильнее слабее сильней слабей тише'.split(
                            ' '):
                        return True

            #повтор формы существительного
            ...
            #наличие в одной строке вариантов с "ё" и с "е" считаем повтором
    ...
    def detect_poor_poetry(self, alignment):
        ...
        if alignment.rhyme_scheme == 'ABAB':
            rhyme_pairs.append(
                (alignment.poetry_lines[0].get_rhyming_tail(), alignment.poetry_lines[2].get_rhyming_tail()))
            rhyme_pairs.append(
                (alignment.poetry_lines[1].get_rhyming_tail(), alignment.poetry_lines[3].get_rhyming_tail()))
        elif alignment.rhyme_scheme == 'ABBA':
            rhyme_pairs.append(
                (alignment.poetry_lines[0].get_rhyming_tail(), alignment.poetry_lines[3].get_rhyming_tail()))
            rhyme_pairs.append(
                (alignment.poetry_lines[1].get_rhyming_tail(), alignment.poetry_lines[2].get_rhyming_tail()))
        elif alignment.rhyme_scheme == 'AABA':
            rhyme_pairs.append(
                (alignment.poetry_lines[0].get_rhyming_tail(), alignment.poetry_lines[1].get_rhyming_tail()))
            rhyme_pairs.append(
                (alignment.poetry_lines[0].get_rhyming_tail(), alignment.poetry_lines[3].get_rhyming_tail()))
            ...
            rhyme_pairs.append(
                (alignment.poetry_lines[0].get_rhyming_tail(), alignment.poetry_lines[1].get_rhyming_tail()))
            rhyme_pairs.append(
                (alignment.poetry_lines[2].get_rhyming_tail(), alignment.poetry_lines[3].get_rhyming_tail()))
        ...
        for tail1, tail2 in rhyme_pairs:
            ...
            if word1.poetry_word.upos == 'VERB' and word2.poetry_word.upos == 'VERB':
                #если пара слов внесена в специальный список рифмующихся слов, то считаем,
                # что тут все нормально:  ВИТАЮ-ТАЮ
                if (form1, form2) in self.accentuator.rhymed_words:
                    continue

                if any((form1.endswith(e) and form2.endswith(e)) for e in
                       'ли ла ло л м шь т тся у те й ю ь лись лась лось лся тся ться я шись в'.split(' ')):
                    return True
        ...
        for bad_group in ['твой мой свой'.split(), 'тебе мне себе'.split(), 'него его'.split(), 'твои свои'.split(),
            ...
            if bad_group[0][0] == '~':
                ...
                for last_word in last_words:
                    n_hits += sum(
                        (re.match('^.+' + ending[1:] + '$', last_word, flags=re.I) is not None) for ending in bad_group)
            else:
    ...
    for true_markup, true_scheme in true_markups:
        try:
            ...
            if alignment.rhyme_scheme != true_scheme:
                ...
                print('Poem:\n{}\nExpected scheme={}\nPredicted scheme={}'.format(true_markup, true_scheme,
                                                                                  alignment.rhyme_scheme))
                exit(0)
